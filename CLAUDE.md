...
LLM provider history, in order: direct Gemini API → OpenRouter (free model with no
`response_format` support) → back to direct Gemini → **OpenRouter again**, on
`nvidia/nemotron-3-super-120b-a12b:free` (real structured-output support). The final switch
was because direct Gemini kept returning 503 "high demand" even on a fresh, correctly-billed
key — a live reliability problem with the provider, not something a new key or a config fix
resolves. See `lib/openrouter.ts`.
...
The LLM returns **facts only** — budget, timeline, service, decision-maker status. It never
...
An LLM call inside the request handler will trigger retries mid-demo.
